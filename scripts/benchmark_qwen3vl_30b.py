"""Benchmark Qwen3-VL-30B-A3B on the fashion dataset (Lightning AI / GPU VM).
...
Usage:
    ...
    # Individual phases
    python scripts/benchmark_qwen3vl_30b.py --phase train
    python scripts/benchmark_qwen3vl_30b.py --phase eval
    python scripts/benchmark_qwen3vl_30b.py --phase latency

    # Eval with custom adapter path
    python scripts/benchmark_qwen3vl_30b.py --phase eval --adapter models/fashion-qwen3vl-30b-a3b/adapter

    # Quick training test (10 steps)
    python scripts/benchmark_qwen3vl_30b.py --phase train --max-steps 10
"""
...
REPORT_DIR = Path("reports/v3_qwen3vl_30b")
TRAINING_CONFIG = Path("configs/training_fashion_qwen3vl_30b.yaml")
SERVING_CONFIG = Path("configs/serving_fashion_qwen3vl_30b.yaml")
...
def run_train(max_steps: int | None = None) -> Path:
    """Fine-tune Qwen3-VL-30B-A3B on the fashion dataset."""
    ...
    console = Console()
    console.print("\n[bold]{'=' * 60}[/bold]")
    console.print("[bold]Phase A: Training Qwen3-VL-30B-A3B[/bold]")
...
def run_eval(adapter_path: str | None = None) -> None:
    ...
    if adapter_path is None:
        adapter_path = "models/fashion-qwen3vl-30b-a3b/adapter"
    ...
    # Print results table
    table = Table(title=f"V3 Qwen3-VL-30B-A3B Results ({len(records)} samples)")
    ...
    if V2_RESULTS.exists():
        ...
        delta_table.add_column("V3 (30B-A3B)", justify="right")
...
def run_latency() -> None:
    ...
    adapter_dir = Path("models/fashion-qwen3vl-30b-a3b/adapter")
    if not adapter_dir.exists():
        console.print(f"[red]Adapter not found at {adapter_dir}[/red]")
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark Qwen3-VL-30B-A3B on fashion dataset"
    )
    parser.add_argument(
        "--phase",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to LoRA adapter (for eval phase, default: models/fashion-qwen3vl-30b-a3b/adapter)",
    )
    parser.add_argument(
        "--max-steps",
        type=int,
    ...
    console = Console()
    console.print("[bold]Qwen3-VL-30B-A3B Fashion Benchmark[/bold]")
